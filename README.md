...
| 屏 | 状态 | 备注 |
|---|---|---|
| **01 待机首页** | ✅ 完整实现 | 三联表盘 + 出发按钮 + 14 天时间线 + **读真实上次跑步** |
| **02 跑前预热** | ✅ 完整实现 | 倒数环 3→2→1→出发 + 4 项系统检查 + 触觉反馈 |
| **03 跑步进行中** | ✅ 完整实现 | 实时计时 + 模拟配速/距离/心率 + 长按 1.5s 暂停 |
| **04 结束总结** | ✅ 完整实现 | AI 解读 + 路线小图 + 配速曲线 + HR 区间环 + 操作按钮 |
| **05 分享卡** | ✅ 完整实现 | 经典 / 极简 / 海报 / 数据 4 样式 2x2 gallery |
| **12 暂停状态** | ✅ 完整实现 | 冻结数据 + 继续/结束 + 二次确认 + 跳到 PostRun |
| 06-08 数据页 | ⏳ v0.4 | |
| 09 AI 教练 | ⏳ v0.4 | |
| 10 设置 | ⏳ v0.4 | |
| 11/16 深入页 | ⏳ v0.4 | |
...
```
首页 [出发] → 跑前预热（3-2-1 倒数）→ 跑步中
            → 长按 1.5 秒 → 暂停页 → 「结束」二次确认
            → 结束总结页（路线 + 数据 + AI 解读）
            → 「分享」→ 4 样式选择
            → 「关闭」→ 回到首页（首页「上次」线显示刚刚跑的真实数据 ✓）
```

### 数据持久化（v0.3 新加）

- 接入 `@react-native-async-storage/async-storage`
- `historyStore` 用 zustand persist 中间件自动同步本地存储
- 每次跑完写入一条 `Run` 记录：起止时间 / 距离 / 时长 / 平均配速 / HR / 5 段 splits
- 首页「上次跑步」一行实时读取最新一条
- 关掉 App 重开数据仍在

### Mock 模式说明

v0.3 仍用模拟传感器数据。真实 GPS / Apple Watch HealthKit / TTS 语音播报在 v0.5 「真实传感器集成」那一轮替换。
